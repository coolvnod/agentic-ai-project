...
import { drawWalls, drawDividerWall, drawZoneLabel, drawBackground, drawNightOverlay, drawRoomTint } from '@/sprites/decorations';
...
function renderFloorLayer(
  ...
  for (let row = 0; row < MAP_ROWS; row++) {
    for (let col = 0; col < MAP_COLS; col++) {
      ...
      drawIsometricTile(ctx, { col, row }, color, '#8B95A040');

      // subtle inner floor seam for richer tilemap readability
  ...
  // Room identity overlays (light tints)
  drawRoomTint(ctx, 12, 16, 1, 4, 'rgba(172, 142, 104, 0.1)'); // boss office
  drawRoomTint(ctx, 12, 16, 6, 9, 'rgba(116, 154, 191, 0.14)'); // meeting room
  drawRoomTint(ctx, 18, 22, 1, 5, 'rgba(186, 160, 121, 0.12)'); // break room
  drawRoomTint(ctx, 18, 22, 7, 10, 'rgba(58, 89, 117, 0.2)'); // server room
  drawRoomTint(ctx, 18, 22, 12, 16, 'rgba(128, 105, 173, 0.16)'); // lounge
...
function renderUILayer(
  ...
  ctx.fillRect(8, 8, 210, 24);
  ctx.strokeStyle = 'rgba(255,255,255,0.16)';
  ctx.strokeRect(8.5, 8.5, 209, 23);
  ctx.restore();
  ...
  ctx.textAlign = 'left';
  ctx.fillText('🏢 AI Office Ops', 16, 24);
  ctx.restore();
  ...
  if (config.connected) {
    ...
    ctx.fillText('● Live', width - 16, 24);
  } else if (config.demoMode) {
    ctx.fillStyle = '#FF9800';
    ctx.fillText('● Demo', width - 16, 24);
  } else {
    ctx.fillStyle = '#F44336';
    ctx.fillText('● Offline', width - 16, 24);
  }
  ctx.restore();

  // Tiny day/night progress meter
  ctx.save();
  const meterW = 118;
  const meterX = width - meterW - 16;
  const meterY = 30;
  ctx.fillStyle = 'rgba(255,255,255,0.14)';
  ctx.fillRect(meterX, meterY, meterW, 4);
  ctx.fillStyle = state.dayNightPhase > 0.5 ? '#90CAF9' : '#FFCC80';
  ctx.fillRect(meterX, meterY, meterW * Math.max(0, Math.min(1, state.dayNightPhase)), 4);
  ctx.restore();
}
