...
export function useAgents(forceDemoMode = false): UseAgentsReturn {
  ...
  const [renamedAgents, setRenamedAgents] = useState<Record<string, string>>({});
  ...
  const applyRenamedName = useCallback((agent: AgentConfig): AgentConfig => {
    const nextName = renamedAgents[agent.id];
    return nextName ? { ...agent, name: nextName } : agent;
  }, [renamedAgents]);
  ...
  const fetchSessions = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      const newAgents = sortedSessions.map((sess, i) => applyRenamedName(sessionToAgentConfig(sess, i)));
      setAgents([...newAgents, ...customAgents.map(applyRenamedName)]);
  ...
  }, [applyRenamedName, customAgents, forceDemoMode]);
  ...
  useEffect(() => {
    ...
    const demoAgents = [...DEMO_AGENTS, ...customAgents].map(applyRenamedName);
    for (const agent of demoAgents) {
      states[agent.id] = generateDemoAgentState(agent.id);
      states[agent.id].statusSummary = agent.id.startsWith('custom-')
        ? 'Manual agent waiting for work'
        : 'Demo agent waiting for work';
    }
    ...
    demoTimerRef.current = setInterval(() => {
      setAgentStates((prev) => {
        const next = { ...prev };
        const demoPool = Object.keys(next);
        const agent = demoPool.length > 0
          ? { id: demoPool[Math.floor(Math.random() * demoPool.length)] }
          : null;
    ...
    feedTimerRef.current = setInterval(() => {
      const event = generateDemoEvent(demoAgents);
  ...
  }, [applyRenamedName, customAgents, demoMode]);
  ...
  const addAgent = useCallback((name: string) => {
    ...
    const index = (agents.length + customAgents.length) % AGENT_AVATARS.length;
    const agent: AgentConfig = {
      id,
      name: trimmed,
      emoji: AGENT_EMOJIS[index % AGENT_EMOJIS.length],
      color: AGENT_COLORS[index % AGENT_COLORS.length],
      avatar: AGENT_AVATARS[index],
      sessionKind: 'unknown',
    };
  ...
  const renameAgent = useCallback((agentId: string, name: string) => {
    ...
    setRenamedAgents((prev) => ({ ...prev, [agentId]: trimmed }));
    setAgents((prev) => prev.map((agent) => (agent.id === agentId ? { ...agent, name: trimmed } : agent)));
    setCustomAgents((prev) => prev.map((agent) => (agent.id === agentId ? { ...agent, name: trimmed } : agent)));
  }, []);
